...
func Update(ctx context.Context, opts UpdateOptions) (*UpdateResult, error) {
	if runtime.GOOS == "windows" {
		return nil, fmt.Errorf("Windows không hỗ trợ tự cập nhật tại chỗ, vui lòng truy cập https://github.com/%s/releases để tải bản mới", opts.Repo)
	}
	if opts.Repo == "" {
...
func selectAsset(rel *release, binaryName string) (releaseAsset, error) {
	...
	return releaseAsset{}, fmt.Errorf("không tìm thấy gói cài đặt cho nền tảng hiện tại *%s trong release %s", suffix, rel.TagName)
}
...
func assetSuffix() (string, error) {
	...
	case "linux":
		osName = "Linux"
	default:
		return "", fmt.Errorf("hệ điều hành %s không được hỗ trợ", runtime.GOOS)
	}
	var arch string
	...
	case "arm64":
		arch = "arm64"
	default:
		return "", fmt.Errorf("kiến trúc %s không được hỗ trợ", runtime.GOARCH)
	}
