...
func (s *askUserState) choiceDescription(idx int) string {
	...
	return "Không có gợi ý nào phù hợp, tự bổ sung"
}
...
func renderAskUserModal(width, height int, state *askUserState) string {
	...
	title := fmt.Sprintf("Cần bổ sung %d/%d", state.index+1, len(state.request.questions))
	...
	if state.typing || (q.MultiSelect && state.selected[len(q.Options)]) {
		b.WriteString("\n")
		b.WriteString(panelTitleStyle.Render("Bổ sung"))
		b.WriteString("\n")
		content := state.input
		if content == "" {
			content = "Nhập vào..."
	...
	if q.MultiSelect {
		hint = "↑↓ Chọn · Space Tick · Enter Nộp · Esc Đóng"
	}
	if state.typing {
		hint = "Nhập bổ sung · Enter Xác nhận · Esc Quay lại"
	}
	b.WriteString("\n")
